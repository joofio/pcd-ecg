...
"""A code converts an SVG file into a CSV file containing the waveform data.
Input: SVG file.
This code includes the following functions: (1) extract the waveform data by parsing the SVG file, (2) change the coordinates
to Cartesian coordinates, (3) adjust the baseline and scale, and (4) linear interpolation
...
def fix_data(x_list, y_list):
    ...
    if dis_fn6 > 0:
        ...
        y_list[6] = list(map(lambda y: y - dis_fn6, y_list[6]))
        x_list[6], y_list[6] = inter(x_list[6], y_list[6])
        y_list[6] = list(map(lambda y: y - y_list[6][0], y_list))
        x_list[6], y_list[6] = inter(x_list[6], y_list[6])
        y_list[6] = list(map(lambda y: y - y_list[6][0], y_list[6]))
    ...
    try:
        ...
        try:
            ...
            (
                ...
                p_df_14,
                p_df_16,
            ) = mk_pECG(x_list, y_list)

            # change to csv file for each part_ECG!
            p_df_3.to_csv(
                date + time + "_" + date + str(int(time) + 3) + "_" + "ECGI" + ".gz",
                index=False,
                header=False,
                compression="gzip",
            )
            p_df_4.to_csv(
                date + time + "_" + date + str(int(time) + 3) + "_" + "ECGII" + ".gz",
                index=False,
                header=False,
                compression="gzip",
            )
            p_df_5.to_csv(
                date + time + "_" + date + str(int(time) + 3) + "_" + "ECGIII" + ".gz",
                index=False,
                header=False,
                compression="gzip",
            )
            p_df_6.to_csv(
                date + time + "_" + date + str(int(time) + 3) + "_" + "ECGaVR" + ".gz",
                index=False,
                header=False,
                compression="gzip",
            )
            p_df_7.to_csv(
                date + time + "_" + date + str(int(time) + 3) + "_" + "ECGaVL" + ".gz",
                index=False,
                header=False,
                compression="gzip",
            )
            p_df_8.to_csv(
                date + time + "_" + date + str(int(time) + 3) + "_" + "ECGaVF" + ".gz",
                index=False,
                header=False,
                compression="gzip",
            )
            p_df_9.to_csv(
                date + time + "_" + date + str(int(time) + 3) + "_" + "ECGV1" + ".gz",
                index=False,
                header=False,
                compression="gzip",
            )
            p_df_10.to_csv(
                date + time + "_" + date + str(int(time) + 3) + "_" + "ECGV2" + ".gz",
                index=False,
                header=False,
                compression="gzip",
            )
            p_df_11.to_csv(
                date + time + "_" + date + str(int(time) + 3) + "_" + "ECGV3" + ".gz",
                index=False,
                header=False,
                compression="gzip",
            )
            p_df_12.to_csv(
                date + time + "_" + date + str(int(time) + 3) + "_" + "ECGV4" + ".gz",
                index=False,
                header=False,
                compression="gzip",
            )
            p_df_13.to_csv(
                date + time + "_" + date + str(int(time) + 3) + "_" + "ECGV5" + ".gz",
                index=False,
                header=False,
                compression="gzip",
            )
            p_df_14.to_csv(
                date + time + "_" + date + str(int(time) + 3) + "_" + "ECGV6" + ".gz",
                index=False,
                header=False,
                compression="gzip",
            )
            p_df_16.to_csv(
                date + time + "_" + date + str(int(time) + 10) + "_" + "ECGII" + ".gz",
                index=False,
                header=False,
                compression="gzip",
            )
